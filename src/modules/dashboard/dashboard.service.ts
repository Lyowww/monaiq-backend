...
import type { AiWarning, AutoInsight, DashboardSummary, SmartSuggestion } from '@ai-finance/shared-types';
...
export class DashboardService {
  ...
  async getSummary(userId: string): Promise<DashboardSummary> {
    ...
    const liquidBalanceMinor =
      this.pickTotal(balanceTotals, 'credit') - this.pickTotal(balanceTotals, 'debit');
    ...
    return {
      ...
      recentTransactions: recentTransactions.map((transaction) => mapTransaction(transaction)),
      aiWarnings: this.buildAiWarnings(scheduledNotes),
      lastInsightAt: user?.lastInsightAt?.toISOString(),
      upcomingPayments: upcomingPayments.map((p) => mapScheduledPayment(p)),
      recurringPayments: recurringPayments.map((p) => mapScheduledPayment(p)),
      debtsIOwe: debtsIOwe.map((d) => mapDebt(d)),
      debtsTheyOweMe: debtsTheyOweMe.map((d) => mapDebt(d)),
      smartSuggestions,
      autoInsights
    };
  }

  private buildAutoInsights(input: {
    liquidBalanceMinor: number;
    monthlyInflowMinor: number;
    monthlyOutflowMinor: number;
    threshold: number;
    activeDebts: DebtDocument[];
    scheduledPending: ScheduledPaymentDocument[];
    in30: number;
    out30: number;
  }): AutoInsight[] {
    ...
    if (monthlyInflowMinor > 0 && monthlyOutflowMinor / monthlyInflowMinor > 0.92) {
      out.push({
        ...
        message:
          'This month, expenses are very close to or above income. Pause discretionary spending until a buffer returns.',
  ...
  private async aggregateTopMerchants(
    userId: Types.ObjectId,
    since: Date
  ): Promise<SmartSuggestion['recentMerchants']> {
  ...
  private async aggregateCategorySpend(
    userId: Types.ObjectId,
    since: Date
  ): Promise<SmartSuggestion['topCategories']> {
    const rows = await this.transactionModel
  ...
  private async aggregatePocketNets(
    userId: Types.ObjectId
  ): Promise<{ cardNet: number; cashNet: number }> {
    const rows = await this.transactionModel
  ...
  private aggregateTotals(userId: Types.ObjectId, bookedAtGte?: Date): Promise<SumByDirection[]> {
  ...
  private pickTotal(items: SumByDirection[], direction: 'credit' | 'debit'): number {
    return items.find((item) => item._id === direction)?.total ?? 0;
  }

  private buildAiWarnings(notes: NoteDocument[]): AiWarning[] {
    return notes
      ...
      .map((note) => {
        ...
        const severity =
          deficitMinor >= 100_000 ? 'critical' : deficitMinor >= 40_000 ? 'high' : 'medium';
  ...
  private calculateDebtPressureScore(debts: DebtDocument[]): number {
    if (debts.length === 0) {
      return 0;
    }

    const relationshipWeights: Record<DebtDocument['relationship'], number> = {
      family: 1,
      friend: 0.92,
      fintech: 0.8,
      bank: 0.72,
    ...
    const totalScore = debts.reduce((score, debt) => {
      const daysUntilDue = Math.ceil(
        (debt.dueDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
      );
      const urgencyScore =
        daysUntilDue <= 0 ? 35 : daysUntilDue <= 7 ? 28 : daysUntilDue <= 30 ? 18 : 8;
      const interestScore = Math.min((debt.aprPercent / 45) * 30, 30);
      const exposureScore = Math.min(
        (debt.outstandingMinor / Math.max(debt.minimumDueMinor, 1)) * 4,
        20
      );
